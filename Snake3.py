...
class SNAKE(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=0, high=ENV_Y,shape=(42,), dtype=np.float64)
    ...
    def get_state(self):
        ...
        last_30 = list(np.array(self.body[-15:]).flatten())
        this_len = len(last_30)
        if ( this_len < 30):
            last_30.extend([0 for i in range(30-this_len)])
