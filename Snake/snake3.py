...
class SNAKE(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=0, high=1,shape=(MAX_SNAKE_LENGTH * 2 + 12,), dtype=np.float64)
    ...
    def get_state(self):
        ...
        last_30 = list(np.array(self.body).flatten() / max(ENV_X, ENV_Y) )
        ...
        output = np.array(
            ...
            [
            self.direction / 3,
            len(self.body) / MAX_SNAKE_LENGTH,
            head_x / ENV_X, 
            head_y / ENV_Y,
            self.apple_x / ENV_X, 
            self.apple_y / ENV_Y,
